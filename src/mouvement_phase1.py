...
def get_liste_case_inconnu_plus_proche_hitman(
    x: int, y: int, n: int, m: int, know: HitmanKnowledge
) -> List[Tuple[int, int]]:
...
def get_action(
    ...
    pos2: Tuple[int, int],
    orientation: HC,
    know: HitmanKnowledge,
    hr: HitmanReferee,
) -> Tuple[List[Callable[[], None]], HC]:
...
def chemin_to_action(
    chemin: List[Tuple[int, int]],
    orientation_depart: HC,
    know: HitmanKnowledge,
    hr: HitmanReferee,
) -> List[Callable[[], None]]:
...
def nbr_obstacle(a: State, b: State, know: HitmanKnowledge) -> int:
    ...
    if a[0] == b[0]:
        for i in range(min(a[1], b[1]), max(a[1], b[1])):
            if know.has_knowledge(a[0], i) and know.knowledge[(a[0], i)] in [
                HC.GUARD_E,
                HC.GUARD_N,
                HC.GUARD_S,
                HC.GUARD_W,
        ...
        for i in range(min(a[0], b[0]), max(a[0], b[0])):
            if know.has_knowledge(i, a[1]) and know.knowledge[(i, a[1])] in [
                HC.GUARD_E,
                HC.GUARD_N,
                HC.GUARD_S,
                HC.GUARD_W,
