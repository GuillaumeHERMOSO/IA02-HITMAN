...
class HitmanKnowledge:
    ...
    def maj_vision_civil(self):
        self.matrice_vision_civil = {
            (i, j): 0 for i in range(self.n) for j in range(self.m)
        }
        for position in self.knowledge:
            vision = []
            if self.knowledge[position] in [
    ...
    def add_knowledge(self, position: Tuple[int, int], content: HC):
        if (not self.has_knowledge(position[0], position[1])) or self.knowledge[
            position
        ] == HC.N:
        ...
        for j in range(self.m):
            for i in range(self.n):
                if not self.has_knowledge(
                    i, j
                ):  # Si on a pas de connaissances, c'est une variable à tester
...
def knowledge_to_clause_personne(
    Dico_know: Dict[Tuple[int, int], HC], dict_var_to_num: Dict[str, int]
) -> ClauseBase:
    ...
    for i in range(hc.n):
        ...
        for j in range(hc.m):
            if (i, j) == (x, y):  # rajout d'Hitman ( vue de haut )
                ...
                tableau[i].append("X")
        # print(tableau[i])
    tableau_resultat = ajouter_zeros_autour(
        transpose_tableau(tableau)
    )  # tableau bien mis ( (0,m) = en haut a gauche , (n,m) = en haut a droite )
    tab_finale = []
    x_1 = 0
    y_1 = 0
        ...
        for col in range(len(tableau_resultat[li])):
            if tableau_resultat[li][col] in [
                ...
                if tableau_resultat[li][col] == HC.GUARD_S:  # Sud
                    ...
                    tab_finale[li][col] = nv_val
                    tab_finale[li][col - 1] = "<"
            elif tableau_resultat[li][col] in [
                HC.CIVIL_N,
                HC.CIVIL_E,
                HC.CIVIL_S,
                HC.CIVIL_W,
